...
private:
  inline void run_task(Task *task) {
    ...
    task->average_execution_time = (task->average_execution_time_alpha * execution_time) +
                                   (1.f - task->average_execution_time_alpha) * task->average_execution_time;
    if (task->max_execution_time < execution_time)
